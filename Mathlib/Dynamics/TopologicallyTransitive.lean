...
[TopologicalSpace α] [AddAction M α] :
    Prop where
  exists_dense_orbit_oneLE : ∃ x : α, Dense (AddAction.orbit (AddSubmonoid.nonneg M) x)
...
[TopologicalSpace α] [MulAction M α] :
    Prop where
  exists_dense_orbit_oneLE : ∃ x : α, Dense (MulAction.orbit (Submonoid.oneLE M) x)
...
theorem MulAction.exists_dense_orbit_oneLE [IsTopologicallyTransitive M α] :
    ∃ x : α, Dense (orbit (Submonoid.oneLE M) x) :=
...
theorem exists_dense_Range_smul_oneLE [IsTopologicallyTransitive M α] :
    ∃ x : α, DenseRange fun c : (Submonoid.oneLE M) ↦ c • x :=
  MulAction.exists_dense_orbit_oneLE M

@[to_additive]
instance (priority := 100) MulAction.isTopologicallyTransitive_of_pretransitive
    [IsPretransitive M α] : IsTopologicallyTransitive M α :=
